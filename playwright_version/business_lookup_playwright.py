...
from web_scraper_playwright import search_ontario_business_playwright
import re
...
def extract_company_info(html_content: str) -> Dict[str, str]:
    ...
    try:
        ...
        if not result_block:
            ...
            return info if 'info' in locals() else {}
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python business_lookup_playwright.py 'Business Name' [output_file.txt]")
        sys.exit(1)
    
    search_term = sys.argv[1]
    default_output = get_output_path('business_lookup_results_playwright.txt') if SAVE_DEBUG_FILES else 'business_lookup_results_playwright.txt'
    output_file = sys.argv[2] if len(sys.argv) > 2 else default_output
    
    print(f"Starting Playwright-based search for: {search_term}")
    
    # Step 1: Search the Ontario Business Registry using Playwright
    html_content = search_ontario_business_playwright(search_term)
