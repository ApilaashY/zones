#!/usr/bin/env python3
"""
Demo script to show the configurable file output behavior for Playwright version.
...
def demo_configuration():
    ...
    print("=" * 60)
    print("PLAYWRIGHT BUSINESS LOOKUP - CONFIGURATION DEMO")
    print("=" * 60)
    ...
    if SAVE_DEBUG_FILES:
        ...
        print(f"   ✅ Owner lookup results in: {OUTPUT_FOLDER}/owner_lookups_playwright/")
    else:
    ...
    print(f"     python process_geojson_owners_playwright.py")
    
    print(f"\n5. OUTPUT FILES:")
    if SAVE_DEBUG_FILES:
        print(f"   Main results: {OUTPUT_FOLDER}/business_lookup_results_playwright.txt")
        print(f"   Debug HTML: {OUTPUT_FOLDER}/last_search_results.html")
        print(f"   Search page: {OUTPUT_FOLDER}/search_results_page.html")
        print(f"   Owner reports: {OUTPUT_FOLDER}/owner_lookups_playwright/business_lookup_details_playwright_*.txt")
    else:
        print(f"   Main results: business_lookup_results_playwright.txt (current directory)")
        print(f"   Owner reports: owner_lookups_playwright/business_lookup_details_playwright_*.txt")
