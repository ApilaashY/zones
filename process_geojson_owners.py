...
from business_lookup import search_ontario_business, extract_company_info, is_company_match, save_results
...
def process_owners(owners: Set[str], output_dir: str = 'owner_lookups') -> Dict[str, Dict]:
    """Process a list of owners and perform business lookups."""
    ...
    details_file = os.path.join(output_dir, f'business_lookup_details_{timestamp}.txt')
    
    with open(details_file, 'w', encoding='utf-8') as details_f:
        # Write header
        details_f.write("=" * 80 + "\n")
        details_f.write("BUSINESS LOOKUP DETAILS - COMPREHENSIVE REPORT\n")
        details_f.write("=" * 80 + "\n")
        details_f.write(f"Generated: {time.strftime('%Y-%m-%d %H:%M:%S')}\n")
        details_f.write(f"Total owners to process: {len(owners)}\n")
        details_f.write("=" * 80 + "\n\n")
        
        for i, owner in enumerate(tqdm(sorted(owners), desc="Processing owners"), 1):
            try:
                # Skip empty or very short names
                ...
                # Perform the search and extract company info
                html_content = search_ontario_business(owner)
                ...
                try:
                    ...
                    from business_lookup import extract_detailed_info_from_cleaned_file
                ...
                if raw_html:
                    from business_lookup import extract_company_details
...
def main():
    ...
    confirm = input("\nDo you want to proceed with the lookups? (y/n): ")
    if confirm.lower() != 'y':
